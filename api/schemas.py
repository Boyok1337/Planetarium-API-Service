...
from api.serializers.planetarium_serializers import AstronomyShowSerializer, ShowSessionListSerializer, \
    PlanetariumDomeSerializer, TicketListSerializer
...
class AstronomyShowSchema:
    list = extend_schema(
        parameters=[
            OpenApiParameter(
                name="show_theme",
                description="Filter by show theme name "
                            "(ex. ?show_theme=Mars)",
                required=False,
...
class ShowSessionSchema:
    list = extend_schema(
        ...
        responses={
            200: ShowSessionListSerializer(many=True)
        }
    )
...
class PlanetariumDomeSchema:
    list = extend_schema(
        ...
        responses={
            200: PlanetariumDomeSerializer(many=True)
        }
    )
...
class ReservationSchema:
    list = extend_schema(
        ...
        responses={
            ...
            403: OpenApiResponse(description="Forbidden if the user does not have permission")
        }
    )
...
class ShowSessionUploadSchema:
    show_session_upload_schema = extend_schema(
        ...
        responses={
            201: OpenApiResponse(
                description='Successfully created show sessions'
            ),
            400: OpenApiResponse(
